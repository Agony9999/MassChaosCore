...
bool Player::CanSelectGobject(uint32 id)
{
    if (IsAdmin())
        return true;

    QueryResult result = WorldDatabase.PQuery("SELECT accId FROM gameobject WHERE guid = %u", id);
    if (!result)
    ...
    uint32 accGM = AccountMgr::GetSecurity(accId);

    if (!accGM && (GetSession()->GetSecurity()))
        return true;
