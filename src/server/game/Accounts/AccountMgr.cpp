...
uint8 GetReportsCount(uint32 accountId)
{    
    PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_CHECK_ACCREPORTS);
    stmt->setUInt32(0, accountId);
    PreparedQueryResult result = LoginDatabase.Query(stmt);
    uint8 reportsCount = 0;
    ...
    {       
        ...
        {   
            fields = result->Fetch();
            uint8 reportWeight = fields[0].GetUInt8();       
            reportsCount += reportWeight;    
        }
        while (result->NextRow()); //Assume that duplicate reports will not make it into the database.
    }*/

    if (result)
    {
        Field* fields = result->Fetch();
        reportsCount = fields[0].GetUInt8();
    }
