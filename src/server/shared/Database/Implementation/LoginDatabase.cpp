...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    //account_reports structure: uint32 accEntry, uint32 reporterEntry, uint8 reportWeight    
    PREPARE_STATEMENT(LOGIN_SEL_CHECK_ACCREPORTS, "SELECT COUNT(accEntry) FROM account_reports WHERE accEntry = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_INS_ACC_REPORT, "INSERT INTO account_reports (accEntry,  reporterEntry, reportWeight) VALUES (?, ?, ?)", CONNECTION_ASYNC);
}
