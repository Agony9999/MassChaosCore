...
public:
    ...
    {		
        ...
        if (DisableMgr::IsDisabledFor(DISABLE_TYPE_ZONE, player->GetZoneId(), player) && !player->IsAdmin())
        {
        ...
        if (DisableMgr::IsDisabledFor(DISABLE_TYPE_GOBJECT, id, player) && !player->IsAdmin())
        {
			handler->PSendSysMessage("This gameobject (id %u) is disabled.", id);
			return true;
        }
		...
		if (save && handler->GetSession()->GetSecurity() < SEC_MODERATOR)
		{
            handler->SendSysMessage("You can only make permament spawns if you are a voter.");
			return true;
		}
    ...
    static bool HandleGameObjectAddTempCommand(ChatHandler* handler, char const* args)
    {
        ...
        // fill the gameobject data and save to the db
        object->SaveToDB(map->GetId(), (1 << map->GetSpawnMode()), player->GetPhaseMaskForSpawn());
        WorldDatabase.PExecute("UPDATE gameobject SET temp=1 WHERE guid = %u", guidLow);
    ...
    {      
        ...
        if (!player->CanSelectGobject(obj->GetGUIDLow()))
        {
            handler->SendSysMessage("You can't select this gameobject.");
            return true;
        }
