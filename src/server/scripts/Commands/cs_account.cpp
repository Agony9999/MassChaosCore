...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "expansion",      SEC_MODERATOR,      false, &HandleAccountAddonCommand,        "", NULL },
            ...
            { "setvoter",       SEC_PLAYER,         false, &HandleAccountSetVoterCommand,     "", NULL },
    ...
    static bool HandleAccountSetVoterCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;

        char* account = (char*)args;
        ...
        QueryResult result = LoginDatabase.PQuery("SELECT vp FROM account_data WHERE id = %u", accountId);
        if (!result)
            return false;
        ...
        if (vp < 112)
        {
            handler->PSendSysMessage("Account has less than 112 votepoints (have %u). Aborting.", vp);
            return true;
        }

        uint32 newvp = vp - 112;

        LoginDatabase.PExecute("INSERT INTO account_access (id, gmlevel) VALUES (%u, 1)", accountId);
        LoginDatabase.PExecute("UPDATE account_data SET vp=%u where id=%u", newvp, accountId);
        handler->PSendSysMessage("Account %s is now a voter and has %u votepoints left.", accountName, newvp);
        return true;
    }
};
