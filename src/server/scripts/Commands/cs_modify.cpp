...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand modifyCommandTable[] =
        {
			...
			{ "emote",          SEC_GAMEMASTER,     false, &HandleModifyEmoteCommand,         "", NULL },
    ...
    static bool HandleModifyFactionCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (DisableMgr::IsDisabledFor(DISABLE_TYPE_ZONE, player->GetZoneId(), player) && target->GetTypeId() != TYPEID_PLAYER)
        {
            handler->SendSysMessage("You can't modify NPC factions in this zone.");
            return true;
        }
    ...
    static bool HandleModifyScaleCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
            if (Player* playerTarget = target->ToPlayer())
            {
                ...
                {
                    handler->PSendSysMessage("%s has modify disabled. You can't use commands on them.", playerTarget->GetName());
                    return true;
                }
        ...
        {
            if (DisableMgr::IsDisabledFor(DISABLE_TYPE_ZONE, player->GetZoneId(), player))
            {
                handler->SendSysMessage("You can't scale NPCs in this zone.");
                return true;
            }
        }
    ...
    static bool HandleModifyMorphCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (DisableMgr::IsDisabledFor(DISABLE_TYPE_DISPLAYID, display_id, player))
        {
			handler->PSendSysMessage("This displayId ('%u') is disabled.", display_id);
			return true;
        }
...
//demorph player or unit
    ...
    static bool HandleMountCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (DisableMgr::IsDisabledFor(DISABLE_TYPE_DISPLAYID, modelid, player))
        {
			handler->PSendSysMessage("This displayId ('%u') is disabled.", modelid);
			return true;
        }
    ...
    static bool HandleModifyLevelCommand(ChatHandler* handler, const char *args)
    {
        ...
        if (newlevel > 80 && !handler->GetSession()->GetPlayer()->IsAdmin())
            newlevel = 80;
    ...
    static bool HandleModifyEmoteCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
