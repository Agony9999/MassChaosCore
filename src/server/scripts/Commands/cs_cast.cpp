...
public:
    ...
    static bool HandleCastDestCommand(ChatHandler* handler, char const* args)
    {
        ...
        {
            if (!caster->ToPlayer()->GetCommandStatus(TOGGLE_MODIFY) && !handler->GetSession()->GetPlayer()->IsAdmin() && handler->GetSession()->GetPlayer()->GetGUID() != caster->GetGUID())
            {
                handler->PSendSysMessage("%s has modify disabled. You can't use commands on them.", caster->GetName());
                return true;
            }
        }
