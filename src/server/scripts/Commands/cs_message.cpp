...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        static ChatCommand commandTable[] =
        {
            ...
            { "announce",       SEC_MODERATOR,      true,   &HandleNameAnnounceCommand,         "", NULL },
            { "gmannounce",     SEC_MODERATOR,      true,   &HandleGMNameAnnounceCommand,       "", NULL },
            { "sysannounce",    SEC_MODERATOR,      true,   &HandleAnnounceCommand,             "", NULL },
            { "gmsysannounce",  SEC_MODERATOR,      true,   &HandleGMAnnounceCommand,           "", NULL },
    ...
    static bool HandleNameAnnounceCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (WorldSession* session = handler->GetSession())
        {
            ...
            nameLink = playerLink(name);
            annColor = session->GetPlayer()->GetAnnounceColor();
        }

        sWorld->SendWorldText(strid, nameLink, annColor.c_str(), args);
        return true;
    }
